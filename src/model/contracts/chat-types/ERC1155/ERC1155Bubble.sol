...
contract ERC1155Bubble is ChatBubble {
  ...
  constructor(IERC1155 nft, uint256 id, address _owner) {
    nftContract = nft;
    nftId = id;
    proxyOwner = _owner;
  }
  ...
  function getAccessPermissions(address user, uint256 contentId) override public view returns (uint256) {
    ...
    else if (user == owner && contentId == 0) return WRITE_BIT;
    else if (contentId == 0) return NO_PERMISSIONS;
    else if (contentId != 0 && nftContract.balanceOf(user, nftId) > 0) return directoryBit | READ_BIT | APPEND_BIT;
    else return NO_PERMISSIONS;
  }

  function canDelete(address _user) external view returns (bool) {
    return _user == owner || _user == proxyOwner;
  }
